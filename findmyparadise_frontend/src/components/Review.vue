...
export default {
  ...
  axios
      ...
      .then((res) => {
        ...
        const totalRatings = this.reviewList.reduce((acc, { rating }) => acc += Number(rating), 0);
        this.overallRating = totalRatings/this.reviewList.length; 

        const totalCleanlinessRt = this.reviewList.reduce((acc, { cleanlinessRating }) => acc += Number(cleanlinessRating), 0);
        this.averageCleanliness = totalCleanlinessRt/this.reviewList.length; 

        const totalCommunicationRt = this.reviewList.reduce((acc, { communicationRating }) => acc += Number(communicationRating), 0);
        this.averageCommunication = totalCommunicationRt/this.reviewList.length; 

        const totalCheckInRt = this.reviewList.reduce((acc, { checkInRating }) => acc += Number(checkInRating), 0);
        this.averageCheckIn = totalCheckInRt/this.reviewList.length; 

        const totalAccuracyRt = this.reviewList.reduce((acc, { accuracyRating }) => acc += Number(accuracyRating), 0);
        this.averageAccuracy = totalAccuracyRt/this.reviewList.length; 

        const totalLocationRt = this.reviewList.reduce((acc, { locationRating }) => acc += Number(locationRating), 0);
        this.averageLocation = totalLocationRt/this.reviewList.length; 


        const totalValueRt = this.reviewList.reduce((acc, { valueRating }) => acc += Number(valueRating), 0);
